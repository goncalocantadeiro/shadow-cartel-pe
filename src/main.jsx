...
const MENU_ITEMS = [
  {id:'skills', title:'Habilidades', description:'Árvore de habilidades e testes do servidor.', icon:Network, color:'#00f0a8', enabled:false},
  {id:'market', title:'Mercado Ilegal', description:'Artigos, contratos e valores do mercado.', icon:ShoppingBag, color:'#ff405f', enabled:false},
  ...
  {id:'laundry', title:'Lavagem', description:'Cálculos de lavagem e dinheiro limpo.', icon:BriefcaseBusiness, color:'#ffb338', enabled:false},
  {id:'coins', title:'F4 Coins', description:'Materiais e ferramentas compradas com moedas.', icon:Coins, color:'#bd7bff', enabled:false},
  {id:'materials', title:'Materiais & Cálculos', description:'Calculadoras rápidas de materiais necessários.', icon:Sparkles, color:'#00e7d7', enabled:false}
];
...
function MainMenu({session,onOpen,onLogout}){
 return <div className="menuPage"><header className="menuTop"><div><p className="kicker">Bem-vindo ao</p><h1>Puerto Esperanza</h1><span>Escolhe o sistema que pretendes abrir</span></div><div className="userBadge"><div><b>{session.username}</b><small>{session.label}</small></div><button onClick={onLogout} title="Terminar sessão"><LogOut size={17}/></button></div></header>
 <div className="menuCards">{MENU_ITEMS.map(item=>{const Icon=item.icon; return <button key={item.id} className={`menuCard ${item.enabled?'':'disabled'}`} style={{'--accent':item.color}} onClick={()=>item.enabled&&onOpen(item.id)}>
   <i><Icon size={27}/></i><h2>{item.title}</h2><p>{item.description}</p><span>{item.enabled?'Abrir':'Em breve'} <b>›</b></span>
 </button>})}</div>
...
async function seedRemote(){
 ...
 for(const table of initialTables){
   for(const craft of table.crafts){
     const row = {name:craft.name, table_name:table.name, output_qty:craft.output, sort_order:order++, image_url: craft.imageUrl || ''};
...
function App(){
 ...
 async function refresh(label='Online'){
  if(!supabase) return;
  try{ const data=await fetchRemote(); setPrices(data.prices); setTables(data.tables); setMarkup(data.markup); setSync(label); }
  catch(e){ console.error(e); setSync('Erro BD'); }
 }
 async function saveMarkup(v){
  if(!isAdmin) return; setMarkup(v); if(supabase){ setSync('A guardar...'); await supabase.from('settings').upsert({id:'sale_percentage', value:Number(v)||0}); setSync('Guardado'); } }
 ...
 async function addCraft(){
  ...
  let craft={id:safeId(), name, output:1, imageUrl:'', ingredients:[]};
  if(supabase){
    setSync('A guardar...');
    const row = {name, table_name:newCraft.table, output_qty:1, sort_order:Date.now(), image_url:''};
    let res = await supabase.from('crafts').insert(row).select('id').single();
 ...
 async function updateCraft(tableName, craftIndex, nextCraft){
  ...
  if(supabase && nextCraft.id){
    setSync('A guardar...');
    const row = {name:nextCraft.name, output_qty:Number(nextCraft.output)||1, table_name:tableName, image_url: nextCraft.imageUrl || ''};
   ...
   <main className="grid">
    ...
    <section className="content">
      <div className="toolbar panel">
        ...
        <div className="markupBox"><span>Venda geral</span><input type="number" value={markup} disabled={!isAdmin} onChange={e=>saveMarkup(e.target.value)}/><b>%</b></div>
      ...
      {visibleTables.map(t=><section className="craftSection" key={t.name} style={{'--accent':t.color}}>
        ...
        <div className="cards">{t.crafts.map(x=><button className="card" key={x.craft.id||x.craft.name} onClick={()=>setSelected(x)}>
          ...
          <div className="cardPrices"><span>Custo {money(x.cost)}</span><strong>Venda {money(sell(x.cost))}</strong></div></div>
        </button>)}</div>
      </section>)}
    </section>
   </main>

   {selected&&<CraftModal readOnly={!isAdmin} selected={selected} prices={prices} materialNames={ingredientNames} priceOf={calc.priceOf} calc={calc} markup={markup} sell={sell} onClose={()=>setSelected(null)} onSave={updateCraft} onDelete={deleteCraft}/>}   
   <footer>{supabase ? <Cloud size={14}/> : <CloudOff size={14}/>} Shadow Cartel // Craft Price Calculator</footer>
 </div>
}

function CraftModal({selected, materialNames, priceOf, calc, markup, sell, onClose, onSave, onDelete, readOnly}){
 ...
 return <div className="modal" onClick={onClose}><div className="detail" onClick={e=>e.stopPropagation()} style={{'--accent':selected.color}}>
    ...
    <div className="priceGrid"><div><small>Custo unitário</small><b>{money(cost)}</b></div><div><small>Venda +{markup}%</small><b>{money(sell(cost))}</b></div><div><small>Custo total do craft</small><b>{money(totalRaw)}</b></div><label><small>Unidades que saem</small>{readOnly?<b>{draft.output}</b>:<input type="number" min="1" value={draft.output} onChange={e=>setDraft({...draft,output:Number(e.target.value)||1})}/>}</label></div>
