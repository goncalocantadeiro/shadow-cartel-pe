...
import { Search, RotateCcw, Plus, Trash2, Save, X } from 'lucide-react';
import './style.css';
...
function App(){
 ...
 useEffect(()=>localStorage.setItem('shadow-prices-v2',JSON.stringify(prices)),[prices]);
 useEffect(()=>localStorage.setItem('shadow-tables-v2',JSON.stringify(tables)),[tables]);
 useEffect(()=>localStorage.setItem('shadow-markup',String(markup)),[markup]);
 ...
 function addMaterial(){
  ...
  if(!name) return;
  setPrices({...prices,[name]:Number(newMat.price)||0});
  setNewMat({name:'', price:0});
 }
 ...
 function addCraft(){
  ...
  setTables(tables.map(t=>t.name===newCraft.table?{...t, crafts:[...t.crafts,{name, output:1, ingredients:[]}]}:t));
  setNewCraft({...newCraft, name:''});
 }
 function updateCraft(tableName, craftIndex, nextCraft){
  const nextTables = tables.map(t=>t.name===tableName?{...t, crafts:t.crafts.map((c,i)=>i===craftIndex?nextCraft:c)}:t);
  setTables(nextTables);
  const t = nextTables.find(t=>t.name===tableName);
  const updated = t?.crafts[craftIndex];
 ...
 function deleteCraft(item){
  if(!confirm(`Apagar craft "${item.craft.name}"?`)) return;
  setTables(tables.map(t=>t.name===item.table?{...t, crafts:t.crafts.filter((_,i)=>i!==item.craftIndex)}:t));
  setSelected(null);
 }
 function resetAll(){
  if(!confirm('Repor dados iniciais? Vais perder alterações guardadas neste browser.')) return;
 ...
 return <div className="app">
   <header className="hero compact">
    ...
    <div className="status"><span></span>Sistema Online</div>
   </header>
   ...
   <main className="grid">
    <aside className="panel materials">
      ...
      {materialNames.map(m=><label key={m} className="matRow"><span>{m}</span><input type="number" value={prices[m]} onChange={e=>setPrices({...prices,[m]:Number(e.target.value)})}/><em>$</em><button type="button" className="mini danger" onClick={()=>removeMaterial(m)}><Trash2 size={13}/></button></label>)}
      </div>
    ...
    <section className="content">
      <div className="toolbar panel">
        ...
        <div className="markupBox"><span>Venda geral</span><input type="number" value={markup} onChange={e=>setMarkup(e.target.value)}/><b>%</b></div>
      ...
      {visibleTables.map(t=><section className="craftSection" key={t.name} style={{'--accent':t.color}}>
        ...
        <div className="cards">{t.crafts.map(x=><button className="card" key={x.craft.name} onClick={()=>setSelected(x)}>
   ...
   <footer>Shadow Cartel // Craft Price Calculator</footer>
 </div>
}
