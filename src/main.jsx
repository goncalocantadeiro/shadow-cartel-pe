...
const MENU_ITEMS = [
  ...
  {id:'craft', title:'Craft', description:'Calcula materiais e quantidades para produzir cada item.', icon:BriefcaseBusiness, color:'#ff405f', enabled:true},
...
function dbRowsToTables(crafts, ingredients, tableRows=[]){
 ...
 (crafts||[]).forEach(c=>{
   ...
   groups.get(tableName).crafts.push({
     ...
     salePriceOverride:c.sale_price_override === null || c.sale_price_override === undefined ? null : Number(c.sale_price_override),
...
async function seedRemote(){
 ...
 for(const table of initialTables){
   for(const craft of table.crafts){
     const row = {name:craft.name, table_name:table.name, output_qty:craft.output, sale_percentage:Number(craft.salePercentage ?? 25), sale_price_override:craft.salePriceOverride ?? null, sort_order:order++, image_url: craft.imageUrl || ''};
...
function App(){
 ...
 const [tables,setTables]=useState(()=>load('shadow-tables-v2', initialTables).map(t=>({...t,crafts:(t.crafts||[]).map(c=>({...c,salePercentage:Number(c.salePercentage ?? 25),salePriceOverride:c.salePriceOverride ?? null}))})));
 ...
 const sell = (cost, percentage=25, override=null) => (override !== null && override !== undefined && override !== '' ? Number(override)||0 : cost * (1 + Number(percentage||0)/100));
 ...
 async function addCraft(){
  ...
  let craft={id:safeId(), name, output:1, salePercentage:25, salePriceOverride:null, imageUrl:'', ingredients:[]};
  if(supabase){
    setSync('A guardar...');
    const row = {name, table_name:newCraft.table, output_qty:1, sale_percentage:25, sale_price_override:null, sort_order:Date.now(), image_url:''};
    let res = await supabase.from('crafts').insert(row).select('id').single();
 ...
 async function updateCraft(tableName, craftIndex, nextCraft){
  ...
  if(supabase && nextCraft.id){
    setSync('A guardar...');
    const row = {name:nextCraft.name, output_qty:Number(nextCraft.output)||1, sale_percentage:Number(nextCraft.salePercentage)||0, sale_price_override:nextCraft.salePriceOverride === '' || nextCraft.salePriceOverride === null || nextCraft.salePriceOverride === undefined ? null : Number(nextCraft.salePriceOverride)||0, table_name:tableName, image_url: nextCraft.imageUrl || ''};
 ...
 if(screen==='menu') return <MainMenu session={session} onOpen={setScreen} onLogout={logout}/>;
 if(screen==='craft') return <CraftCalculatorScreen session={session} isAdmin={isAdmin} tables={tables} calc={calc} sell={sell} sync={sync} supabase={supabase} onRefresh={()=>refresh('Atualizado')} onMenu={()=>setScreen('menu')} onLogout={logout} materialNames={ingredientNames} onSave={updateCraft} onDelete={deleteCraft}/>;
    ...
    <section className="content">
      ...
      {visibleTables.map(t=><section className="craftSection" key={t.name} style={{'--accent':t.color}}>
        ...
        <div className="cards">{t.crafts.map(x=><button className="card" key={x.craft.id||x.craft.name} onClick={()=>setSelected(x)}>
          ...
          <div className="cardPrices"><span>Custo {money(x.cost)}</span><strong>Venda {money(sell(x.cost,x.craft.salePercentage,x.craft.salePriceOverride))}</strong></div></div>
        </button>)}</div>
      </section>)}
    </section>
   </main>
...
function CraftCalculatorScreen({session,isAdmin,tables,calc,sell,sync,supabase,onRefresh,onMenu,onLogout,materialNames,onSave,onDelete}){
 const [query,setQuery]=useState('');
 const [activeTable,setActiveTable]=useState('Todas');
 const [selectedCalc,setSelectedCalc]=useState(null);
 const all=useMemo(()=>tables.flatMap(t=>(t.crafts||[]).map((craft,craftIndex)=>({table:t.name,tag:t.tag,color:t.color,craft,craftIndex,cost:calc.unitCost(craft),missing:calc.missingFor(craft)}))),[tables,calc]);
 const filtered=all.filter(x=>(activeTable==='Todas'||x.table===activeTable)&&x.craft.name.toLowerCase().includes(query.toLowerCase()));
 const visible=tables.map(t=>({...t,items:filtered.filter(x=>x.table===t.name)})).filter(t=>activeTable==='Todas'||t.name===activeTable);
 return <div className="app craftCalcPage">
  <header className="hero compact"><div><p className="kicker">Sistema de Craft</p><h1>Shadow Cartel Craft</h1><span>Calcula as quantidades necessárias para produzir cada item</span></div><div className="headActions"><button className="navBtn" onClick={onMenu}><ArrowLeft size={15}/> Menu</button><div className="status"><span></span>{sync}{supabase&&<button className="syncBtn" onClick={onRefresh}>Atualizar</button>}</div><button className="navBtn" onClick={onLogout}><LogOut size={15}/> Sair</button></div></header>
  <main className="craftCalcContent">
   <div className="toolbar panel craftToolbar"><div className="search"><Search size={16}/><input placeholder="Procurar item..." value={query} onChange={e=>setQuery(e.target.value)}/></div><div className="tabs"><button className={activeTable==='Todas'?'on':''} onClick={()=>setActiveTable('Todas')}>Todas</button>{tables.map(t=><button key={t.name} className={activeTable===t.name?'on':''} onClick={()=>setActiveTable(t.name)}>{t.tag}</button>)}</div></div>
   {visible.map(t=><section className="craftSection calcSection" key={t.name} style={{'--accent':t.color}}><div className="sectionHead"><span>{t.tag}</span><h2>{t.name}</h2><small>{t.items.length} itens</small></div><div className="calcCards">{t.items.map(x=><button className="calcCard" key={x.craft.id||x.craft.name} onClick={()=>setSelectedCalc(x)}><div className="corner"></div><div className="calcThumb">{x.craft.imageUrl?<img src={x.craft.imageUrl} alt="" loading="lazy"/>:<span>{x.craft.name.slice(0,2).toUpperCase()}</span>}</div><h3>{x.craft.name}</h3><p>{x.craft.ingredients.length?`${x.craft.ingredients.length} ingredientes`:'Falta receita'}</p><strong>Calcular</strong></button>)}</div></section>)}
  </main>
  {selectedCalc&&<CraftCalculatorModal selected={selectedCalc} isAdmin={isAdmin} materialNames={materialNames} calc={calc} sell={sell} onClose={()=>setSelectedCalc(null)} onSave={onSave} onDelete={onDelete}/>} 
  <footer><Cloud size={14}/> Shadow Cartel // Craft Calculator</footer>
 </div>
}

function CraftCalculatorModal({selected,isAdmin,materialNames,calc,sell,onClose,onSave,onDelete}){
 const [draft,setDraft]=useState(()=>JSON.parse(JSON.stringify(selected.craft)));
 const [wanted,setWanted]=useState(1);
 useEffect(()=>{setDraft(JSON.parse(JSON.stringify(selected.craft)));setWanted(1)},[selected.craft.id,selected.craft.name]);
 const craftsNeeded=Math.max(1,Math.ceil((Number(wanted)||1)/Math.max(1,Number(draft.output)||1)));
 const requirements=(draft.ingredients||[]).map(([name,qty])=>({name,qty:Number(qty||0)*craftsNeeded,unit:Number(qty||0)}));
 ...
 const unitCost=calc.unitCost(draft);
 return <div className="modal" onClick={onClose}><div className="detail calculatorDetail" onClick={e=>e.stopPropagation()} style={{'--accent':selected.color}}><button className="close" onClick={onClose}><X/></button><p className="kicker">{selected.table}</p><div className="calcModalTitle"><div className="calcModalImage">{draft.imageUrl?<img src={draft.imageUrl} alt=""/>:<span>{draft.name.slice(0,2).toUpperCase()}</span>}</div><div><h2>{draft.name}</h2><small>Produz {draft.output} unidade{Number(draft.output)!==1?'s':''} por craft</small></div></div>
  <div className="quantityBox"><label>Quantidade que queres produzir<input type="number" min="1" value={wanted} onChange={e=>setWanted(Math.max(1,Number(e.target.value)||1))}/></label><div><small>Crafts necessários</small><b>{craftsNeeded}</b></div><div><small>Custo total</small><b>{money(unitCost*(Number(wanted)||1))}</b></div><div><small>Venda total</small><b>{money(sell(unitCost,draft.salePercentage,draft.salePriceOverride)*(Number(wanted)||1))}</b></div></div>
  <div className="modalHead"><h3>Materiais necessários</h3>{isAdmin&&<button onClick={addIng}><Plus size={15}/> Adicionar ingrediente</button>}</div>
  {requirements.length===0?<div className="empty">Ainda não tens a receita deste item.</div>:<table><thead><tr><th>Material/Craft</th><th>Por craft</th><th>Total necessário</th><th></th></tr></thead><tbody>{requirements.map((r,idx)=><tr key={idx}><td>{isAdmin?<select value={draft.ingredients[idx][0]} onChange={e=>setIng(idx,[e.target.value,draft.ingredients[idx][1]])}>{materialNames.map(n=><option key={n}>{n}</option>)}</select>:r.name}</td><td>{isAdmin?<input type="number" min="0" value={draft.ingredients[idx][1]} onChange={e=>setIng(idx,[draft.ingredients[idx][0],Number(e.target.value)||0])}/>:r.unit}</td><td><strong>{r.qty}</strong></td><td>{isAdmin&&<button className="mini danger" onClick={()=>removeIng(idx)}><Trash2 size={13}/></button>}</td></tr>)}</tbody></table>}
  <div className="modalActions calcActions">{isAdmin&&<><button className="dangerBtn" onClick={()=>onDelete(selected)}><Trash2 size={15}/> Apagar item</button><button className="save" onClick={()=>onSave(selected.table,selected.craftIndex,draft)}><Save size={16}/> Guardar receita</button></>}</div>
 </div></div>
}
...
function CraftModal({selected, materialNames, priceOf, calc, sell, onClose, onSave, onDelete, readOnly}){
 ...
 return <div className="modal" onClick={onClose}><div className="detail" onClick={e=>e.stopPropagation()} style={{'--accent':selected.color}}>
    ...
    <div className="priceGrid priceGridSix"><div><small>Custo unitário</small><b>{money(cost)}</b></div><div><small>Venda calculada +{Number(draft.salePercentage||0)}%</small><b>{money(cost*(1+Number(draft.salePercentage||0)/100))}</b></div><div><small>Venda final</small><b>{money(sell(cost,draft.salePercentage,draft.salePriceOverride))}</b></div><div><small>Custo total do craft</small><b>{money(totalRaw)}</b></div><label><small>Unidades que saem</small>{readOnly?<b>{draft.output}</b>:<input type="number" min="1" value={draft.output} onChange={e=>setDraft({...draft,output:Number(e.target.value)||1})}/>}</label><label><small>Percentagem de venda</small>{readOnly?<b>{Number(draft.salePercentage||0)}%</b>:<div className="percentInput"><input type="number" min="0" value={draft.salePercentage??25} onChange={e=>setDraft({...draft,salePercentage:Number(e.target.value)||0,salePriceOverride:null})}/><span>%</span></div>}</label></div>
    {!readOnly&&<div className="roundingBar"><label><span>Preço final manual</span><input type="number" min="0" placeholder="Automático" value={draft.salePriceOverride??''} onChange={e=>setDraft({...draft,salePriceOverride:e.target.value===''?null:Number(e.target.value)})}/></label><span>Arredondar:</span>{[5,10,50].map(step=><button key={step} type="button" onClick={()=>setDraft({...draft,salePriceOverride:Math.round((cost*(1+Number(draft.salePercentage||0)/100))/step)*step})}>{step}$</button>)}<button type="button" className="clearRound" onClick={()=>setDraft({...draft,salePriceOverride:null})}>Automático</button></div>}
