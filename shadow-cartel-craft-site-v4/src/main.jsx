...
function App(){
 ...
 return <div className="app">
   <header className="hero compact">
    ...
    <div className="status"><span></span>{sync}{supabase&&<button className="syncBtn" onClick={()=>refresh('Atualizado')}>Atualizar</button>}</div>
   </header>
   ...
   <main className="grid">
    <aside className="panel materials">
      ...
      {sideTab==='base' ? <>
        ...
        {baseMaterialNames.map(m=><label key={m} className="matRow"><span>{m}</span><input type="number" value={prices[m]} onChange={e=>changeMaterial(m,e.target.value)}/><em>$</em><button type="button" className="mini danger" onClick={()=>removeMaterial(m)}><Trash2 size={13}/></button></label>)}
        </div>
      </> : <div className="matList compactList">
   ...
   {selected&&<CraftModal selected={selected} prices={prices} materialNames={ingredientNames} priceOf={calc.priceOf} calc={calc} markup={markup} sell={sell} onClose={()=>setSelected(null)} onSave={updateCraft} onDelete={deleteCraft}/>}   
   <footer>{supabase ? <Cloud size={14}/> : <CloudOff size={14}/>} Shadow Cartel // Craft Price Calculator</footer>
 </div>
}

function CraftModal({selected, materialNames, priceOf, calc, markup, sell, onClose, onSave, onDelete}){
 ...
 return <div className="modal" onClick={onClose}><div className="detail" onClick={e=>e.stopPropagation()} style={{'--accent':selected.color}}>
    ...
    <div className="imageUrl"><span>URL da imagem do item</span><input placeholder="https://..." value={draft.imageUrl||''} onChange={e=>setDraft({...draft,imageUrl:e.target.value})}/></div>
    ...
    {draft.ingredients.length===0?<div className="empty">Ainda não tens a receita deste item.</div>:<table><thead><tr><th>Material/Craft</th><th>Qtd</th><th>Preço usado</th><th>Total</th><th></th></tr></thead><tbody>{draft.ingredients.map(([m,q],idx)=><tr key={idx}><td><select value={m} onChange={e=>setIng(idx,[e.target.value,q])}>{materialNames.map(n=><option key={n}>{n}</option>)}</select></td><td><input type="number" value={q} onChange={e=>setIng(idx,[m,Number(e.target.value)||0])}/></td><td>{money(priceOf(m))}</td><td>{money(priceOf(m)*q)}</td><td><button className="mini danger" onClick={()=>removeIng(idx)}><Trash2 size={13}/></button></td></tr>)}</tbody></table>}
